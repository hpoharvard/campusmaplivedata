...
def routefeed(route_id_val):
    ...
    r = requests.get('https://transloc-api-1-2.p.rapidapi.com/routes.json?callback=call&agencies=52', headers=headers)
    routes = r.json()['data']
    df_routes = pd.DataFrame.from_dict(json_normalize(routes['52']), orient='columns')
...
def stopsHU():   
    try:
        key = "....."
        headers = {'X-Mashape-Key' : key,'Accept' : 'application/json'}
        r = requests.get('https://transloc-api-1-2.p.mashape.com/stops.json?agencies=52&callback=call', headers=headers)
        stops = r.json()['data']

        hu_stops = pd.DataFrame.from_dict(json_normalize(stops), orient='columns')

        # M2 arrival-estimates feed
        key = "....."
        headers = {'X-Mashape-Key' : key,'Accept' : 'application/json'}
        r = requests.get('https://transloc-api-1-2.p.rapidapi.com/arrival-estimates.json?callback=call&agencies=52', headers=headers)
        arrival = r.json()['data']
        hu_arrival = pd.DataFrame.from_dict(json_normalize(arrival), orient='columns')

        if hu_arrival.empty is True:
            print ("No Arrival")
        else:    
            hu_stops_merge = pd.merge(left = hu_stops, right = hu_arrival, how='left', on='stop_id')    
            hu_stops_merge = hu_stops_merge.rename(columns={'code':'code','name':'name','stop_id':'stop_id','arrivals':'arrivals'})    
            df1 = hu_stops_merge[['code','name','stop_id','arrivals']]    
            d = []
            ...
            stops_fset = stops_features.query(where="category_subtype = 'Harvard Shuttle Stops'") #querying without any conditions returns all the features
